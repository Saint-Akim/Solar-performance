...
with tab2:
    ...
    if not filtered.empty and 'total_solar' in filtered.columns:
        st.plotly_chart(go.Figure(go.Scatter(x=filtered['ts'], y=filtered['total_solar'], mode='lines+markers', name='Solar Output')), width='stretch')
        st.success(f"Peak Solar Today: {filtered['total_solar'].max():.1f} kW")
    st.markdown("</div>", unsafe_allow_html=True)
with tab3:
    st.markdown("<div class='fuel-card'>", unsafe_allow_html=True)
    if not factory_df.empty and 'sensor.bottling_factory_monthkwhtotal' in factory_df.columns:
        st.plotly_chart(go.Figure(go.Scatter(x=factory_df['last_changed'], y=factory_df['daily_factory_kwh'], mode='lines+markers', name='Daily Factory kWh')), width='stretch')
    st.markdown("</div>", unsafe_allow_html=True)
with tab4:
    st.markdown("<div class='fuel-card'>", unsafe_allow_html=True)
