...
if st.session_state.theme == 'dark':
    theme = {
        ...
        "label": "#A0A0A0", "border": "1px solid #333",
        "grid": "#333", "accent": "#E74C3C", "success": "#2ECC71"
    }
else:
    theme = {
        "bg": "#F8F9FA", "card": "#FFFFFF", "text": "#2C3E50",
        "label": "#7F8C8D", "border": "1px solid #E9ECEF",
        "grid": "#E9ECEF", "accent": "#E74C3C", "success": "#2ECC71"
    }
            ...
            if col and not df.empty:
                ...
                try:
                    merged = pd.merge_asof(
                        merged, df, left_on='ts', right_on='ts_temp',
                        direction='nearest', tolerance=pd.Timedelta('1h')
                    )
                except Exception as e:
                    st.warning(f"Error merging data: {e}")

# ------------------ POST-PROCESS ------------------
if not merged.empty:
    if 'sensor.fronius_grid_power' in merged.columns: merged['fronius_kw'] = merged['sensor.fronius_grid_power'] / 1000
    if 'sensor.goodwe_grid_power' in merged.columns: merged['goodwe_kw'] = merged['sensor.goodwe_grid_power'] / 1000
...
def fuelsa_chart(df, x, y, title, color):
    ...
    fig.add_trace(go.Scatter(
        x=df[x], y=df[y], mode='lines+markers', name=title,
...
with tab4:
    ...
    try:
        ...
        c1, c2 = st.columns(2)
        with c1:
            from_date = ws['B2'].value
            if isinstance(from_date, str):
                from_date = datetime.strptime(from_date or "30/09/25", "%d/%m/%y").date()
            elif isinstance(from_date, datetime):
                from_date = from_date.date()
            from_date = st.date_input("From", value=from_date)
            c7 = st.number_input("Freedom Village", value=float(ws['C7'].value or 0))
        with c2:
            to_date = ws['B3'].value
            if isinstance(to_date, str):
                to_date = datetime.strptime(to_date or "05/11/25", "%d/%m/%y").date()
            elif isinstance(to_date, datetime):
                to_date = to_date.date()
            to_date = st.date_input("To", value=to_date)
        ...
        st.error(f"Billing template unavailable: {e}")
    st.markdown("</div>", unsafe_allow_html=True)
