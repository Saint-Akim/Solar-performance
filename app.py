# app.py — Southern Paarl Energy Dashboard (Light HA Style)
...
st.set_page_config(
    ...
    page_icon="sununshine",
    layout="wide",
    initial_sidebar_state="expanded"
)

# ------------------ CLEAN LIGHT HA-STYLE CSS (NO DARK MODE) ------------------
st.markdown("""
<style>
...
with st.sidebar:
    ...
    st.markdown("<div style='font-weight:700; font-size:18px'>Hussein Akim</div>", unsafe_allow_html=True)
...
if page == "Solar Performance":
    ...
    fig = make_chart(filtered, 'last_changed', 'sum_grid_power', "Actual Power (kW)", "#00C853")
    ...
    if 'sensor.generator_fuel_consumed' in filtered.columns:
        st.plotly_chart(make_chart(filtered, 'last_changed', 'sensor.generator_fuel_consumed',
                                  "Fuel Consumed (L)", "#DC2626"), use_container_width=True)
    if 'sensor.generator_runtime_duration' in filtered.columns:
        st.plotly_chart(make_chart(filtered, 'last_changed', 'sensor.generator_runtime_duration',
                                  "Runtime (hours)", "#7C3AED"), use_container_width=True)
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    if 'daily_factory_kwh' in filtered.columns:
        st.plotly_chart(make_chart(filtered, 'last_changed', 'daily_factory_kwh',
                                  "Daily kWh", "#0EA5E9"), use_container_width=True)
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    if 'sensor.kehua_internal_power' in filtered.columns:
        st.plotly_chart(make_chart(filtered, 'last_changed', 'sensor.kehua_internal_power',
                                  "Power (kW)", "#06B6D4"), use_container_width=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
# ------------------ FOOTER ------------------
st.markdown("---")
st.markdown("<p style='text-align:center; color:#888; font-size:0.9rem;'>Built with love by Hussein Akim • Southern Paarl • December 2025</p>", unsafe_allow_html=True)
