...
# Fixed: Labels, data filtering, UI consistency, responsiveness, billing preview
...
<style>
    ...
    @media (max-width: 768px) {
        [data-testid="stColumns"] > div {flex-direction: column !important;}
...
with st.sidebar:
    ...
    with col1:
        start_date = st.date_input("From Date", value=datetime(2025, 5, 1))
    with col2:
        end_date = st.date_input("To Date", value=datetime(2025, 5, 31))
...
def load_csvs(urls):
    ...
    for url in urls:
        try:
            ...
            st.warning(f"Failed to load {url.split('/')[-1]}")
...
def plot(df, x, y, title, color):
    if df.empty or y not in df.columns:
        fig = go.Figure()
        fig.add_annotation(text="No data available for selected period", showarrow=False, font=dict(size=16, color="#666"))
    else:
        fig = go.Figure()
...
if page == "Solar":
    ...
    if ws:
        ...
        with col1:
            ...
            from_date = st.date_input("From Date", value=datetime.strptime(b2, "%d/%m/%y").date(), key="from")
            b3 = ws['B3'].value or "05/11/25"
            to_date = st.date_input("To Date", value=datetime.strptime(b3, "%d/%m/%y").date(), key="to")
        with col2:
        ...
        if st.button("Apply Changes & Generate Invoice", type="primary"):
            with st.spinner("Generating updated invoice..."):
